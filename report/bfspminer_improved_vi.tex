...
\makeatletter
\def\abstract{\begin{center}{\bf TÓM TẮT\vspace{-.5em}\vspace{0pt}}\end{center}}
...
\begin{equation}
    L_{max}(t) \!=\! \begin{cases} 
      \max(L_{min}, L_{max}(t\!-\!1) \!-\! 1), & M_{cur} \!>\! \mu_{\text{high}} \\
      \max(L_{min}, L_{max}(t\!-\!1) \!-\! 1), & H \!>\! \theta_{\text{high}} \\
      \min(L_{ceil}, L_{max}(t\!-\!1) \!+\! 1), & H \!<\! \theta_{\text{low}} \\
      L_{max}(t\!-\!1), & \text{khác}
   \end{cases}
\end{equation}
...
\textbf{Hạn chế.} Công trình còn ba hạn chế chính: (i) Precision@3 trên MSNBC mới đạt 51,89\%, cho thấy dư địa cải thiện qua các mô hình dự đoán tiên tiến hơn (attention-based, neural); (ii) chưa so sánh trực tiếp với các thuật toán SPM trên luồng hiện đại do khác biệt cơ bản về giả định đầu vào (batch vs.\ batch-free); và (iii) chỉ đánh giá trên hai tập dữ liệu. Dù vậy, so với baseline BFSPMiner gốc, phương pháp đề xuất tăng 90\% số mẫu phát hiện và +26 điểm phần trăm Precision@3 trên MSNBC, đồng thời duy trì khả năng mở rộng tuyến tính và kiểm soát bộ nhớ hiệu quả.
