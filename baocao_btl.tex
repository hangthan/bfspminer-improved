...
\fancyhead[R]{\small Bài tập lớn -- Khai phá Dữ liệu}
\fancyfoot[C]{\thepage}
...
% --- TRANG BÌA (placeholder - người dùng sẽ tự chuẩn bị) ---
\begin{titlepage}
\centering
\vspace*{1cm}
{\large\textbf{HỌC VIỆN KỸ THUẬT QUÂN SỰ}}\\[0.3cm]
{\large\textbf{VIỆN CÔNG NGHỆ THÔNG TIN \& TRUYỀN THÔNG}}\\[0.3cm]
{\large\textbf{CHƯƠNG TRÌNH THẠC SĨ}}\\[2cm]

\rule{\linewidth}{0.5mm}\\[0.5cm]
{\Huge\bfseries BÁO CÁO BÀI TẬP LỚN}\\[0.3cm]
{\Huge\bfseries KẾT THÚC MÔN HỌC}\\[0.5cm]
\rule{\linewidth}{0.5mm}\\[1cm]

{\LARGE\textbf{Môn học:} Khai phá Dữ liệu (Data Mining)}\\[1cm]

{\Large\textbf{Đề tài:}}\\[0.3cm]
{\Large Nghiên cứu và cải tiến thuật toán BFSPMiner}\\[0.2cm]
{\Large khai phá mẫu tuần tự trên luồng dữ liệu}\\[2cm]

{\large
\begin{tabular}{ll}
\textbf{Sinh viên thực hiện:} & Thân Thị Hằng \\
                              & Lê Thị Kiều Trang \\
                              & Hà Tuấn Hiếu \\[0.5cm]
\textbf{Giáo viên hướng dẫn:} & TS Nguyễn Hoàng Sinh \\
\end{tabular}
}\\[2cm]
...
\section{Đặt vấn đề}
Khai phá mẫu tuần tự (Sequential Pattern Mining -- SPM) trên luồng dữ liệu liên tục là một trong những bài toán quan trọng bậc nhất trong lĩnh vực khai phá dữ liệu hiện đại. Trong thời đại Internet vạn vật (IoT), thương mại điện tử và hệ thống thông tin quy mô lớn, dữ liệu không còn được lưu trữ dưới dạng tĩnh mà liên tục phát sinh dưới dạng \textit{luồng} (stream) --- một chuỗi vô hạn các sự kiện đến với tốc độ cao và phân bố thay đổi theo thời gian.
...
Phần lớn các thuật toán khai phá mẫu tuần tự trên luồng dữ liệu dựa trên mô hình xử lý theo lô (\textit{batch-based}), chia luồng vô hạn thành các khối có kích thước cố định rồi khai phá từng khối. Cách tiếp cận này tồn tại hai vấn đề nghiêm trọng:
...
Thuật toán BFSPMiner (Batch-Free Sequential Pattern Miner) được đề xuất bởi Hassani et al. \cite{hassani2019} đã khắc phục các nhược điểm trên bằng khung khai phá \textit{batch-free} (không chia lô). Tuy nhiên, bản gốc vẫn tồn tại hai hạn chế:
\begin{itemize}
...
\subsection{Bước 1: Nhận item $a$ (t = 0)}
...
\subsection{Bước 2: Nhận item $b$ (t = 1)}
...
\subsection{Bước 3: Nhận item $a$ (t = 2)}
...
\subsection{Bước 4: Nhận item $b$ (t = 3)}
...
\begin{itemize}
    ...
    \item Cập nhật cây:
    \begin{itemize}
        \item Nút \texttt{b} (con root): count $2 \rightarrow 3$
...
\subsection{Bước 5: Nhận item $c$ (t = 4)}
...
\subsection{Trích xuất mẫu phổ biến (min\_support = 0.2)}
...
\begin{table}[H]
    ...
    \begin{tabular}{llccl}
        ...
        root $\rightarrow$ b              & $(b)$         & 3 & 0.60 & $\checkmark$ \\
...
\textbf{Đo lường:} Bộ nhớ đo bằng \texttt{tracemalloc} (peak heap); Thời gian đo bằng \texttt{time.perf\_counter()}.
...
\textbf{Phân tích REDD:} Do tính lặp lại cao, Naive-Majority đã đạt 81.51\%. BFSPMiner-Base đạt 97.92\% và Improved nâng lên 99.45\%, đồng thời phát hiện thêm 93\% số mẫu (105 $\rightarrow$ 203).

\textbf{Phân tích MSNBC:} Đây là kết quả quan trọng nhất. BFSPMiner-Base chỉ đạt 25.65\% --- \textit{thấp hơn cả} Naive-Majority (49.53\%). Điều này cho thấy ràng buộc consecutive nghiêm trọng hạn chế khả năng dự đoán trên luồng entropy cao. BFSPMiner-Improved đạt \textbf{51.89\%}, vượt qua Majority Baseline, đồng thời tăng 90\% số mẫu.
...
Cả hai phương pháp thể hiện \textbf{khả năng mở rộng tuyến tính}. Improved chậm hơn Base khoảng 4--5$\times$ do sinh mẫu gap-aware, nhưng đổi lại +90\% mẫu và +26pp Precision@3.  Tốc độ xử lý trung bình đạt $\sim$7,100 sự kiện/giây --- đủ đáp ứng hầu hết ứng dụng thời gian thực.
...
Toàn bộ mã nguồn được tổ chức trong thư mục \texttt{core/} với các file chính sau. Mã nguồn đầy đủ được đính kèm trong thư mục nộp bài.

\section{core/config.py --- Cấu hình tham số}
\begin{lstlisting}[language=Python, caption={Cau hinh BFSPMiner}]
...
\section{core/inverted\_t0\_tree.py --- Cay Inverted T0}
\begin{lstlisting}[language=Python, caption={Cay Inverted T0 (trich doan)}]
class TreeObject:
    __slots__ = ['value', 'parent', 'time_stamps',
                 'count', 'batch_count', 'tid', 'children']
    def __init__(self, value, parent, batch_time, time=None):
        self.value = value
        self.parent = parent
        self.count = 1
        self.batch_count = 1
        ...
        self.children = {}  # HashMap: O(1) lookup

class InvertedT0Tree:
    def __init__(self, max_pattern_length):
        self.max_pattern_length = max_pattern_length
        self.events = []
        self.head = TreeObject("root", None, 0)
        self.head.count = 0

    def add_event(self, event_label, batch):
        self.events.append(event_label)
        current_time = len(self.events) - 1
        pattern_list = [self.events[current_time - i]
                        for i in range(self.max_pattern_length)
                        if current_time - i >= 0]
        visited_nodes = set()
        self._add_list_to_tree(
            pattern_list, current_time, batch,
            self.head, visited_nodes)
        self.head.count = len(self.events)
\end{lstlisting}

\section{core/bfspminer.py --- Thuat toan chinh}
\begin{lstlisting}[language=Python, caption={BFSPMiner class (trich doan)}]
class BFSPMiner:
    ...
    def feed_item(self, item):
        ...
        if self.config.enable_gap:
            patterns = self.episode_extension.generate_patterns_with_gap(
                self.tree.events, len(self.tree.events), ...)
    ...
    def get_frequent_patterns(self, min_support):
        return self.tree.extract_patterns(
            min_support, max(0, self.processed_items - 1))

    def predict_next(self, k=3, min_support=0.01):
        # Context matching + confidence ranking
        ...
\end{lstlisting}

\section{core/adaptive\_max\_length.py --- Adaptive Length}
\begin{lstlisting}[language=Python, caption={Adaptive maxPatternLength}]
class AdaptiveMaxPatternLength:
    def __init__(self, config):
        self.min_len = config.adaptive_min_len    # 3
        self.max_len = config.adaptive_max_len    # 15
...
\section{core/episode\_extension.py --- Gap-Aware Mining}
\begin{lstlisting}[language=Python, caption={Gap-Aware Episode Mining}]
class EpisodeMiningExtension:
    def __init__(self, max_gap=1, window_size=5):
        self.max_gap = max_gap
        self.window_size = window_size

    def generate_patterns_with_gap(self, events, current_time,
                                   max_pattern_length):
        ...
        max_combinations = 50  # Hard limit
